...
def getGoogleService(service_name, version):
    ...
    credential = None
    if os.path.exists(UTILS_DIR_NAME + '/token_abp.json'):
        credential = Credentials.from_authorized_user_file(UTILS_DIR_NAME + '/token_abp.json', SCOPES)
    if not credential or not credential.valid:
        ...
        with open(UTILS_DIR_NAME + '/token_abp.json', 'w') as token:
...
def getSpreadSheetIdFromLocalFile():
    sheetId = None
    if not os.path.exists(UTILS_DIR_NAME):
        sheetId = None
    else:
        if not os.path.exists(UTILS_DIR_NAME + "/keys.txt"):
            sheetId = None
        else:
            with open(UTILS_DIR_NAME + "/keys.txt", "r") as file:
                sheetId = file.read()
                file.close()
    return sheetId
