...
def update_google_sheets(sheetId, data):
    credential = None
    if os.path.exists('utils/token_pdf.json'):
        credential = Credentials.from_authorized_user_file('utils/token_pdf.json', SCOPES)
    if not credential or not credential.valid:
        ...
        with open('utils/token_pdf.json', 'w') as token:
...
def getSpreadSheetIdFromLocalFile():
    ...
    if not os.path.exists("utils"):
        ...
        if not os.path.exists("utils/keys.txt"):
            ...
            with open("utils/keys.txt", "r") as file:
                sheetId = file.read()
                file.close()
    return sheetId
...
def run():
    ...
    print("Processing...")
    sheetId = getSpreadSheetIdFromLocalFile()
    if sheetId == None:
        print("Something went wrong during read \"Key.txt\" file into <utils> directory.")
    else:
        sheet_data = []
